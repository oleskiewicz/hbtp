...
class HBTReader:
        ...
        if self.nfiles:
            ...
            if filetype == "HaloSize":
                return (
                    self.rootdir
                    + "/HaloSize/"
                    + "HaloSize_%03d.hdf5" % (isnap)
                )
            else:
                return (
                    self.rootdir + "/" + filetype + "Snap_%03d.hdf5" % (isnap)
                )
            ...
            with self.OpenFile(isnap, i, filetype) as subfile:
                if subindex is None:
                    ...
                    if offset + nsub > subindex:
                        subhalos.append(
                            subfile[filetype + "haloParticles"][
                                subindex - offset
                            ]
                        )
                        break
        ...
        for i in range(max(self.nfiles, 1)):
            with self.OpenFile(isnap, i) as subfile:
                ...
                if offset + nsub > subindex:
                    try:
                        return subfile[
                            "ParticleProperties/Sub%d" % (subindex - offset)
                        ][
                            ...
    ...
    def GetSub(self, trackId, isnap=-1):
        ...
        if self.nfiles:
            subid = np.flatnonzero(
                self.LoadSubhalos(isnap, "TrackId") == trackId
            )[0]
        else:
            subid = trackId
    ...
    def GetTrack(self, trackId, fields=None, MaxSnap=None):
        ...
        snaps = []
        scales = []
        snapbirth = self.GetSub(trackId)["SnapshotIndexOfBirth"][0]
    ...
    def GetExclusiveParticles(self, isnap=-1):
        ...
        OriginPart.sort(key=lambda x: len(x[1]))
        repo = set()
        NewPart = []
    ...
    def GetHostMergerTree(self, HostHaloId, isnap=-1, file=sys.stdout):
        ...
        try:
            m = self.LoadHostHalos(isnap, HostHaloId)["M200Crit"][0]
        except:
            m = 0.0

        file.write(
            '\t"%d_%d" [label="%d, %d, %.2f"]\n'
            % (isnap, HostHaloId, isnap, HostHaloId, m)
        )
    ...
    def GetProgenitorHaloes(self, HostHaloId, isnap=-1):
        ...
        try:
            progenitors = set(
                [
                    self.GetSub(subhalo, isnap - 1)[0]["HostHaloId"]
                    for subhalo in self.GetSubsOfHost(HostHaloId, isnap=isnap)[
                        "TrackId"
                    ]
                    if self.GetSub(subhalo, isnap - 1)[0]["Rank"] == 0
                ]
